...
# Example: Make a simple chat completion request using openrouter_client

example_response = openrouter_client.chat.completions.create(
...
print("Example OpenRouter response:", example_response.choices[0].message.content)
# %%
...
def serialize_conversation_chatml(messages: list[dict]) -> str:
    ...
    for message in messages:
        ...
        if role == "assistant" and message.get("tool_calls") and content is None:
            # Assistant message with tool_calls and no textual content
            content = json.dumps(message["tool_calls"])
        elif role == "tool":
            tool_call_id = message.get("tool_call_id")
            role = f"tool(tool_call_id={tool_call_id})"
            content = message.get("content", "")
        else:
            content = message.get("content", "")
...
## [INST] and [/INST] for user messages

# How is the system message handled?
## Is was removed

# What does the token sequence look like compared to ChatML?
## Similar but missing system message and different sturcture and numbers
