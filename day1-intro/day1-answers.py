...
def serialize_conversation_chatml(messages: list[dict]) -> str:
    ...
    for message in messages:
        if message["role"] == "assistant" and message.get("tool_calls") and not message.get("content"):
            content = json.dumps(message["tool_calls"])
        elif message["role"] == "tool":
            content = message["content"]
            message["role"] = f'tool(tool_call_id={message.get("tool_call_id")})'
        else:
            content = message["content"]
        serialized += "<|im_start|>{role}\n{content}<|im_end|>\n".format(role=message["role"], content=content)
    return serialized
...
def get_completion_with_logprobs(
    ...
    token_log_probs_list = openrouter_client.chat.completions.create(
        model=model,
        messages=messages,
    ...
    alts = []
    for entry in token_log_probs_list:
        print([alt for alt in entry[1]])
        print(entry[1][0].toplogprobs())
        alts.append([(alt.token, alt.logprob) for alt in entry])
